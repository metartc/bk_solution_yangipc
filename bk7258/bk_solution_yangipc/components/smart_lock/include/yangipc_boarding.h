#ifndef __YANGIPC_BOARDING_H__
#define __YANGIPC_BOARDING_H__
...
typedef struct
{
    ...
    uint16_t channel;
} yangipc_boarding_info_t;


int yangipc_boarding_init(void);
void yangipc_boarding_event_notify(uint16_t opcode, int status);
void yangipc_boarding_ble_disable(void);
int yangipc_boarding_notify(uint8_t *data, uint16_t length);
