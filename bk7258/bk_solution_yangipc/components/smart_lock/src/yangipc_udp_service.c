#include <common/bk_include.h>

#include <os/mem.h>
#include <os/str.h>
#include <os/os.h>
#include <driver/int.h>
#include <common/bk_err.h>

#ifdef CONFIG_RTT
#include <sys/socket.h>
#endif
#include "lwip/sockets.h"

#include "bk_uart.h"
#include <os/mem.h>

#include "yangipc_comm.h"
#include "yangipc_cmd.h"
#include "yangipc_network.h"
#include "yangipc_udp_service.h"
#include "yangipc_transmission.h"
#include "yangipc_devices.h"
#include "yangipc_audio_device.h"
#include "frame/frame_que_v2.h"
#include "frame_buffer.h"

#define TAG "yangipc-UDP"

#define LOGI(...) BK_LOGI(TAG, ##__VA_ARGS__)
#define LOGW(...) BK_LOGW(TAG, ##__VA_ARGS__)
#define LOGE(...) BK_LOGE(TAG, ##__VA_ARGS__)
#define LOGD(...) BK_LOGD(TAG, ##__VA_ARGS__)
#define LOGV(...) BK_LOGV(TAG, ##__VA_ARGS__)

#define APP_DEMO_UDP_SOCKET_TIMEOUT     100  // ms


typedef struct
{
    beken_thread_t thd;
    struct sockaddr_in img_remote;
    struct sockaddr_in aud_remote;
    int img_fd;
    int aud_fd;
    uint16_t running : 1;
    uint16_t img_status : 1;
    uint16_t aud_status : 1;
    db_channel_t *img_channel;
    db_channel_t *aud_channel;
} db_udp_service_t;


db_udp_service_t *db_udp_service = NULL;


#define UDP_MAX_RETRY (1000)
#define UDP_MAX_DELAY (20)

void yangipc_udp_update_remote_address(in_addr_t address)
{
    LOGD("%s\n", __func__);
    db_udp_service->img_remote.sin_addr.s_addr = address;
    db_udp_service->aud_remote.sin_addr.s_addr = address;
}

int yangipc_udp_img_send_packet(uint8_t *data, uint32_t len)
{
    if (!db_udp_service->img_status)
    {
        return -1;
    }

    return yangipc_socket_sendto(&db_udp_service->img_fd, (struct sockaddr *)&db_udp_service->img_remote, data, len, -sizeof(db_trans_head_t));
}


int yangipc_udp_img_send_prepare(uint8_t *data, uint32_t length)
{
    yangipc_transmission_pack(db_udp_service->img_channel, data, length);

    return 0;
}

void *yangipc_udp_img_get_tx_buf(void)
{
    if (db_udp_service == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_buf, service null\n");
        return NULL;
    }

    if (db_udp_service->img_channel == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_buf, img_channel null\n");
        return NULL;
    }

    LOGD("yangipc_udp_img_get_tx_buf, tbuf %p\n", db_udp_service->img_channel->tbuf);

    return db_udp_service->img_channel->tbuf + 1;
}

int yangipc_udp_img_get_tx_size(void)
{
    if (db_udp_service == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_size, service null\n");
        return 0;
    }

    if (db_udp_service->img_channel == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_size, img_channel null\n");
        return 0;
    }

    return db_udp_service->img_channel->tsize - sizeof(db_trans_head_t);
}


int yangipc_udp_aud_send_packet(uint8_t *data, uint32_t len)
{
    if (!db_udp_service->aud_status)
    {
        return -1;
    }

    return yangipc_socket_sendto(&db_udp_service->aud_fd, (struct sockaddr *)&db_udp_service->aud_remote, data, len, -sizeof(db_trans_head_t));
}

int yangipc_udp_aud_send_prepare(uint8_t *data, uint32_t length)
{
    yangipc_transmission_pack(db_udp_service->aud_channel, data, length);

    return 0;
}

void *yangipc_udp_aud_get_tx_buf(void)
{
    if (db_udp_service == NULL)
    {
        LOGE("yangipc_udp_aud_get_tx_buf, service null\n");
        return NULL;
    }

    if (db_udp_service->aud_channel == NULL)
    {
        LOGE("yangipc_udp_aud_get_tx_buf, aud_channel null\n");
        return NULL;
    }

    //LOGD("yangipc_udp_aud_get_tx_buf, tbuf %p\n", db_udp_service->aud_channel->tbuf);

    return db_udp_service->aud_channel->tbuf + 1;
}

int yangipc_udp_aud_get_tx_size(void)
{
    if (db_udp_service == NULL)
    {
        LOGE("yangipc_udp_aud_get_tx_size, service null\n");
        return 0;
    }

    if (db_udp_service->aud_channel == NULL)
    {
        LOGE("yangipc_udp_aud_get_tx_size, img_channel null\n");
        return 0;
    }

    return db_udp_service->aud_channel->tsize - sizeof(db_trans_head_t);
}

// V2版本：包装函数适配UDP图传接口
static frame_buffer_t *udp_transmission_read_frame(image_format_t format, uint32_t timeout)
{
    // UDP传输是消费者，使用CONSUMER_TRANSMISSION ID
    // 标记为慢速消费者（受网络影响）
    return frame_queue_v2_get_frame(format, CONSUMER_TRANSMISSION, timeout);
}

static void udp_transmission_free_frame(image_format_t format, frame_buffer_t *frame)
{
    // UDP传输释放帧
    frame_queue_v2_release_frame(format, CONSUMER_TRANSMISSION, frame);
}

static media_transfer_cb_t yangipc_udp_img_channel =
{
    .send = yangipc_udp_img_send_packet,
    .prepare = yangipc_udp_img_send_prepare,
    .get_tx_buf = yangipc_udp_img_get_tx_buf,
    .get_tx_size = yangipc_udp_img_get_tx_size,
    .read = udp_transmission_read_frame,
    .free = udp_transmission_free_frame,
};

static const media_transfer_cb_t yangipc_udp_aud_channel =
{
    .send = yangipc_udp_aud_send_packet,
    .prepare = yangipc_udp_aud_send_prepare,
    .get_tx_buf = yangipc_udp_aud_get_tx_buf,
    .get_tx_size = yangipc_udp_aud_get_tx_size,
};

static inline void yangipc_udp_voice_receiver(db_channel_t *channel, uint16_t sequence, uint16_t flags, uint32_t timestamp, uint8_t sequences, uint8_t *data, uint16_t length)
{
    LOGV("%s %d\n", __func__, length);
    yangipc_audio_data_callback(data, length);
}

static void yangipc_udp_service_main(beken_thread_arg_t data)
{
    int maxfd, ret = 0;
    int rcv_len = 0;
    socklen_t srvaddr_len = 0;
    fd_set watchfd;
    struct timeval timeout;
    u8 *rcv_buf = NULL;
    in_addr_t remote = yangipc_cmd_get_socket_address();

    LOGD("yangipc_udp_service, img: %d, aud: %d\n", YANGIPC_UDP_IMG_PORT, YANGIPC_UDP_AUD_PORT);
    (void)(data);

    rcv_buf = (u8 *)os_malloc((YANGIPC_NETWORK_MAX_SIZE + 1) * sizeof(u8));
    if (!rcv_buf)
    {
        LOGE("udp os_malloc failed\n");
        goto out;
    }

    // for data transfer
    db_udp_service->img_fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (db_udp_service->img_fd == -1)
    {
        LOGE("socket failed\n");
        goto out;
    }

    db_udp_service->img_remote.sin_family = AF_INET;
    db_udp_service->img_remote.sin_port = htons(YANGIPC_UDP_IMG_PORT);

    if (remote == 0)
    {
        db_udp_service->img_remote.sin_addr.s_addr = htonl(INADDR_ANY);
    }
    else
    {
        db_udp_service->img_remote.sin_addr.s_addr = remote;
    }

    srvaddr_len = (socklen_t)sizeof(struct sockaddr_in);
    if (bind(db_udp_service->img_fd, (struct sockaddr *)&db_udp_service->img_remote, srvaddr_len) == -1)
    {
        LOGE("bind failed\n");
        goto out;
    }

    db_udp_service->aud_fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (db_udp_service->aud_fd == -1)
    {
        LOGE("vo socket failed\n");
        goto out;
    }

    db_udp_service->aud_remote.sin_family = AF_INET;
    db_udp_service->aud_remote.sin_port = htons(YANGIPC_UDP_AUD_PORT);

    if (remote == 0)
    {
        db_udp_service->aud_remote.sin_addr.s_addr = htonl(INADDR_ANY);
    }
    else
    {
        db_udp_service->aud_remote.sin_addr.s_addr = remote;
    }

    srvaddr_len = (socklen_t)sizeof(struct sockaddr_in);
    if (bind(db_udp_service->aud_fd, (struct sockaddr *)&db_udp_service->aud_remote, srvaddr_len) == -1)
    {
        LOGE("bind failed\n");
        goto out;
    }
    maxfd = (db_udp_service->img_fd > db_udp_service->aud_fd) ? db_udp_service->img_fd : db_udp_service->aud_fd;

    timeout.tv_sec = APP_DEMO_UDP_SOCKET_TIMEOUT / 1000;
    timeout.tv_usec = (APP_DEMO_UDP_SOCKET_TIMEOUT % 1000) * 1000;

    db_udp_service->img_status = 1;
    db_udp_service->running = 1;
    db_udp_service->aud_status = 1;

    {
        yangipc_msg_t msg;

        msg.event = DBEVT_LAN_UDP_SERVICE_START_RESPONSE;
        msg.param = 0;
        yangipc_send_msg(&msg);
    }

    while (db_udp_service->running)
    {
        FD_ZERO(&watchfd);
        FD_SET(db_udp_service->img_fd, &watchfd);
        FD_SET(db_udp_service->aud_fd, &watchfd);

        //DBD("wait for select\n");
        ret = select(maxfd + 1, &watchfd, NULL, NULL, &timeout);
        if (ret < 0)
        {
            LOGE("select ret:%d\n", ret);
            break;
        }
        else
        {
            // is img fd, data transfer
            if (FD_ISSET(db_udp_service->img_fd, &watchfd))
            {
                rcv_len = recvfrom(db_udp_service->img_fd, rcv_buf, YANGIPC_NETWORK_MAX_SIZE, 0,
                                   (struct sockaddr *)&db_udp_service->img_remote, &srvaddr_len);

                if (rcv_len <= 0)
                {
                    // close this socket
                    LOGE("recv close fd:%d, error_code:%d\n", db_udp_service->img_fd, errno);
                    break;
                }
                else
                {
                    rcv_len = (rcv_len > YANGIPC_NETWORK_MAX_SIZE) ? YANGIPC_NETWORK_MAX_SIZE : rcv_len;
                    rcv_buf[rcv_len] = 0;

                    LOGE("demo_yangipc_udp_receiver\n");
                    //demo_yangipc_udp_receiver(rcv_buf, rcv_len, &db_udp_service->img_remote);
                }
            }

            if (FD_ISSET(db_udp_service->aud_fd, &watchfd))
            {
                rcv_len = recvfrom(db_udp_service->aud_fd, rcv_buf, YANGIPC_NETWORK_MAX_SIZE, 0,
                                   (struct sockaddr *)&db_udp_service->aud_remote, &srvaddr_len);

                if (rcv_len <= 0)
                {
                    // close this socket
                    LOGE("recv close fd:%d, error_code:%d\n", db_udp_service->aud_fd, errno);
                    break;
                }
                else
                {
                    rcv_len = (rcv_len > YANGIPC_NETWORK_MAX_SIZE) ? YANGIPC_NETWORK_MAX_SIZE : rcv_len;
                    rcv_buf[rcv_len] = 0;

                    yangipc_transmission_unpack(db_udp_service->aud_channel, rcv_buf, rcv_len, yangipc_udp_voice_receiver);

                }
            }
        }
    }

out:

    LOGE("yangipc_udp_service_main exit %d\n", db_udp_service->running);

#if AUDIO_TRANSFER_ENABLE
    /* deinit audio transfer driver */
    bk_aud_intf_drv_deinit();
#endif

    if (rcv_buf)
    {
        os_free(rcv_buf);
        rcv_buf = NULL;
    }

    if (db_udp_service->img_fd != -1)
    {
        close(db_udp_service->img_fd);
        db_udp_service->img_fd = -1;
    }

    if (db_udp_service->aud_fd != -1)
    {
        close(db_udp_service->aud_fd);
        db_udp_service->aud_fd = -1;
    }

    db_udp_service->img_status = 0;
    db_udp_service->running = 0;
    db_udp_service->aud_status = 0;
    db_udp_service->thd = NULL;
    rtos_delete_thread(NULL);
}

bk_err_t yangipc_udp_service_init(void)
{
    int ret;

    LOGD("%s\n", __func__);

    if (db_udp_service != NULL)
    {
        LOGE("db_udp_service already init\n");
        return BK_FAIL;
    }

    db_udp_service = os_malloc(sizeof(db_udp_service_t));

    if (db_udp_service == NULL)
    {
        LOGE("db_udp_service malloc failed\n");
        goto error;
    }

    os_memset(db_udp_service, 0, sizeof(db_udp_service_t));

    db_udp_service->img_channel = yangipc_transmission_malloc(0, YANGIPC_UDP_NETWORK_MAX_SIZE);

    if (db_udp_service->img_channel == NULL)
    {
        LOGE("img_channel malloc failed\n");
        goto error;
    }

    db_udp_service->aud_channel = yangipc_transmission_malloc(YANGIPC_UDP_NETWORK_MAX_SIZE, YANGIPC_UDP_NETWORK_MAX_SIZE);

    if (db_udp_service->aud_channel == NULL)
    {
        LOGE("aud_channel malloc failed\n");
        goto error;
    }

    yangipc_devices_set_camera_transfer_callback(&yangipc_udp_img_channel);

    yangipc_devices_set_audio_transfer_callback(&yangipc_udp_aud_channel);

    ret = rtos_create_thread(&db_udp_service->thd,
                             4,
                             "yangipc",
                             (beken_thread_function_t)yangipc_udp_service_main,
                             1024 * 2,
                             (beken_thread_arg_t)NULL);
    if (ret != BK_OK)
    {
        LOGE("Error: Failed to create yangipc udp service: %d\n", ret);
        return BK_FAIL;
    }

    LOGD("db_udp_service->img_channel %p\n", db_udp_service->img_channel);
    LOGD("db_udp_service->aud_channel %p\n", db_udp_service->aud_channel);

    return BK_OK;
error:

    if (db_udp_service->aud_channel)
    {
        os_free(db_udp_service->aud_channel);
    }

    if (db_udp_service->img_channel)
    {
        os_free(db_udp_service->img_channel);
    }

    if (db_udp_service)
    {
        os_free(db_udp_service);
    }

    return BK_FAIL;
}

void yangipc_udp_service_deinit(void)
{
    LOGD("%s\n", __func__);

    if (db_udp_service == NULL)
    {
        LOGE("%s service is NULL\n", __func__);
        return;
    }

    if (db_udp_service->aud_channel)
    {
        os_free(db_udp_service->aud_channel);
        db_udp_service->aud_channel = NULL;
    }

    if (db_udp_service->img_channel)
    {
        os_free(db_udp_service->img_channel);
        db_udp_service->img_channel = NULL;
    }

    db_udp_service->running == 0;
    while (db_udp_service->thd)
    {
        rtos_delay_milliseconds(10);
    }

    os_free(db_udp_service);
    db_udp_service = NULL;
}
