#include "bk_private/bk_init.h"
#include <components/system.h>
#include <os/os.h>
#include <components/shell_task.h>

#include <modules/wifi.h>
#include <components/event.h>
#include <components/netif.h>


#include "yangipc_comm.h"
#include "yangipc_sdp.h"
#include "yangipc_cmd.h"
#include "yangipc_devices.h"
#include "yangipc_audio_device.h"
#include "yangipc_network.h"
#include "yangipc_cs2_service.h"
#include "yangipc_udp_service.h"
#include "yangipc_tcp_service.h"
#include "yangipc_boarding.h"
#include "components/bluetooth/bk_dm_bluetooth.h"

#define LOGI(...) BK_LOGI(TAG, ##__VA_ARGS__)
#define LOGW(...) BK_LOGW(TAG, ##__VA_ARGS__)
#define LOGE(...) BK_LOGE(TAG, ##__VA_ARGS__)
#define LOGD(...) BK_LOGD(TAG, ##__VA_ARGS__)
#define LOGV(...) BK_LOGV(TAG, ##__VA_ARGS__)

#define TAG "db-core"

typedef struct
{
    uint32_t enabled : 1;
    uint32_t service : 6;

    char *id;
    beken_thread_t thd;
    beken_queue_t queue;
} yangipc_info_t;

yangipc_info_t *db_info = NULL;

const yangipc_service_interface_t *yangipc_current_service = NULL;

bk_err_t yangipc_send_msg(yangipc_msg_t *msg)
{
    bk_err_t ret = BK_OK;

    if (db_info->queue)
    {
        ret = rtos_push_to_queue(&db_info->queue, msg, BEKEN_NO_WAIT);

        if (BK_OK != ret)
        {
            LOGE("%s failed\n", __func__);
            return BK_FAIL;
        }

        return ret;
    }

    return ret;
}

static void yangipc_message_handle(void)
{
    bk_err_t ret = BK_OK;
    yangipc_msg_t msg;

    while (1)
    {
        ret = rtos_pop_from_queue(&db_info->queue, &msg, BEKEN_WAIT_FOREVER);

        if (kNoErr == ret)
        {
            LOGD("######%s, event:%d\n", __func__, msg.event);
            switch (msg.event)
            {
                case DBEVT_WIFI_STATION_CONNECT:
                {
                    LOGD("DBEVT_WIFI_STATION_CONNECT\n");

                    yangipc_boarding_info_t *yangipc_boarding_info = (yangipc_boarding_info_t *) msg.param;
                    yangipc_wifi_sta_connect(yangipc_boarding_info->boarding_info.ssid_value,
                                              yangipc_boarding_info->boarding_info.password_value);
                }
                break;

                case DBEVT_WIFI_STATION_CONNECTED:
                {
                    LOGD("DBEVT_WIFI_STATION_CONNECTED\n");

                    netif_ip4_config_t ip4_config;
                    extern uint32_t uap_ip_is_start(void);

                    os_memset(&ip4_config, 0x0, sizeof(netif_ip4_config_t));
                    bk_netif_get_ip4_config(NETIF_IF_AP, &ip4_config);
                    if (uap_ip_is_start())
                    {
                        bk_netif_get_ip4_config(NETIF_IF_AP, &ip4_config);
                    }
                    else
                    {
                        bk_netif_get_ip4_config(NETIF_IF_STA, &ip4_config);
                    }

                    LOGD("ip: %s\n", ip4_config.ip);

                    yangipc_boarding_event_notify_with_data(BOARDING_OP_STATION_START, BK_OK, ip4_config.ip, strlen(ip4_config.ip));
                }
                break;

                case DBEVT_WIFI_STATION_DISCONNECTED:
                {
                    LOGD("DBEVT_WIFI_STATION_DISCONNECTED\n");
                }
                break;

                case DBEVT_WIFI_SOFT_AP_TURNING_ON:
                {
                    LOGD("%s, DBEVT_WIFI_SOFT_AP_TURNING_ON, warning: not adapt\n", __func__);
#if 0
                    yangipc_boarding_info_t *yangipc_boarding_info = (yangipc_boarding_info_t *) msg.param;
                    int ret = yangipc_wifi_soft_ap_start(yangipc_boarding_info->boarding_info.ssid_value,
                                                          yangipc_boarding_info->boarding_info.password_value,
                                                          yangipc_boarding_info->channel);

                    if (ret == BK_OK)
                    {
                        yangipc_boarding_event_notify(BOARDING_OP_SOFT_AP_START, EVT_STATUS_OK);
                    }
                    else
                    {
                        yangipc_boarding_event_notify(BOARDING_OP_SOFT_AP_START, EVT_STATUS_ERROR);
                    }
#endif
                }
                break;

                case DBEVT_LAN_UDP_SERVICE_START_REQUEST:
                {
                    LOGD("DBEVT_LAN_UDP_SERVICE_START_REQUEST\n");

                    if (db_info->service != YANGIPC_SERVICE_NONE)
                    {
                        LOGW("DBEVT_LAN_UDP_SERVICE_START_REQUEST service: %d already start up\n", db_info->service);
                        break;
                    }

                    db_info->service = YANGIPC_SERVICE_LAN_UDP;

                    yangipc_cmd_server_init();
                    yangipc_udp_service_init();
                }
                break;

                case DBEVT_LAN_UDP_SERVICE_START_RESPONSE:
                {
                    LOGD("DBEVT_LAN_UDP_SERVICE_START_RESPONSE\n");

                    yangipc_sdp_start("yangipc-udp", YANGIPC_CMD_PORT, YANGIPC_UDP_IMG_PORT, YANGIPC_UDP_AUD_PORT);

                    yangipc_boarding_event_notify(BOARDING_OP_SERVICE_UDP_START, BK_OK);
                }
                break;

                case DBEVT_LAN_TCP_SERVICE_START_REQUEST:
                {
                    LOGD("DBEVT_LAN_TCP_SERVICE_START_REQUEST\n");

                    if (db_info->service != YANGIPC_SERVICE_NONE)
                    {
                        LOGW("DBEVT_LAN_TCP_SERVICE_START_REQUEST service: %d already start up\n", db_info->service);
                        break;
                    }

                    db_info->service = YANGIPC_SERVICE_LAN_TCP;

                    yangipc_cmd_server_init();
                    yangipc_tcp_service_init();

                }
                break;

                case DBEVT_LAN_TCP_SERVICE_START_RESPONSE:
                {
                    LOGD("DBEVT_LAN_TCP_SERVICE_START_RESPONSE\n");

                    yangipc_sdp_start("yangipc-tcp", YANGIPC_CMD_PORT, YANGIPC_TCP_IMG_PORT, YANGIPC_TCP_AUD_PORT);

                    yangipc_boarding_event_notify(BOARDING_OP_SERVICE_TCP_START, BK_OK);
                }
                break;

                case DBEVT_P2P_CS2_SERVICE_START_REQUEST:
                {
#ifdef CONFIG_INTEGRATION_YANGIPC_CS2
                    LOGD("DBEVT_P2P_CS2_SERVICE_START_REQUEST\n");

                    if (db_info->service != YANGIPC_SERVICE_NONE)
                    {
                        LOGW("DBEVT_P2P_CS2_SERVICE_START_REQUEST service: %d already start up\n", db_info->service);
                        break;
                    }

                    db_info->service = YANGIPC_SERVICE_P2P_CS2;

                    p2p_cs2_key_t *key = (p2p_cs2_key_t *)msg.param;

                    yangipc_current_service = get_yangipc_cs2_service_interface();
                    yangipc_current_service->init(key);
#endif
                }
                break;

                case DBEVT_P2P_CS2_SERVICE_START_RESPONSE:
                {
                    yangipc_boarding_event_notify(BOARDING_OP_SRRVICE_CS2_START, BK_OK);
                }
                break;

                case DBEVT_START_BOARDING_EVENT:
                {
                    uint16_t opcode = msg.param & 0xFFFF;
                    int status = msg.param >> 16;
                    yangipc_boarding_event_notify(opcode, status);
                }
                break;


                case DBEVT_BLE_DISABLE:
                {
#if CONFIG_BLUETOOTH_AP
                    bk_bluetooth_deinit();
                    LOGD("close bluetooth finish!\r\n");
#endif
                }
                break;

                case DBEVT_REMOTE_DEVICE_CONNECTED:
                {
                    if (db_info->service == YANGIPC_SERVICE_LAN_UDP)
                    {
                        yangipc_udp_update_remote_address((in_addr_t)msg.param);
                        yangipc_sdp_reload();
                    }
                    else if (db_info->service == YANGIPC_SERVICE_LAN_TCP)
                    {
                        yangipc_sdp_reload();
                    }
                }
                break;

                case DBEVT_REMOTE_DEVICE_DISCONNECTED:
                {
                    yangipc_video_transfer_turn_off();
                    yangipc_audio_turn_off();

                    if (db_info->service == YANGIPC_SERVICE_LAN_UDP)
                    {
                        yangipc_sdp_start("yangipc-udp", YANGIPC_CMD_PORT, YANGIPC_UDP_IMG_PORT, YANGIPC_UDP_AUD_PORT);
                    }
                    else if (db_info->service == YANGIPC_SERVICE_LAN_TCP)
                    {
                        yangipc_sdp_start("yangipc-tcp", YANGIPC_CMD_PORT, YANGIPC_TCP_IMG_PORT, YANGIPC_TCP_AUD_PORT);
                    }
                }
                break;

                case DBEVT_IMAGE_TCP_SERVICE_DISCONNECTED:
                {
                    yangipc_video_transfer_turn_off();

                    if (db_info->service == YANGIPC_SERVICE_LAN_UDP)
                    {
                        yangipc_sdp_start("yangipc-udp", YANGIPC_CMD_PORT, YANGIPC_UDP_IMG_PORT, YANGIPC_UDP_AUD_PORT);
                    }
                    else if (db_info->service == YANGIPC_SERVICE_LAN_TCP)
                    {
                        yangipc_sdp_start("yangipc-tcp", YANGIPC_CMD_PORT, YANGIPC_TCP_IMG_PORT, YANGIPC_TCP_AUD_PORT);
                    }
                }
                break;

                case DBEVT_EXIT:
                    goto exit;
                    break;

                default:
                    break;
            }
        }
    }

exit:

    yangipc_sdp_stop();

    /* delate msg queue */
    ret = rtos_deinit_queue(&db_info->queue);

    if (ret != kNoErr)
    {
        LOGE("delate message queue fail\n");
    }

    db_info->queue = NULL;

    LOGE("delate message queue complete\n");

    /* delate task */
    rtos_delete_thread(NULL);

    db_info->thd = NULL;

    LOGE("delate task complete\n");
}


void yangipc_core_init(void)
{
    bk_err_t ret = BK_OK;

    if (db_info == NULL)
    {
        db_info = os_malloc(sizeof(yangipc_info_t));

        if (db_info == NULL)
        {
            LOGE("%s, malloc db_info failed\n", __func__);
            goto error;
        }

        os_memset(db_info, 0, sizeof(yangipc_info_t));
    }


    if (db_info->queue != NULL)
    {
        ret = BK_FAIL;
        LOGE("%s, db_info->queue allready init, exit!\n", __func__);
        goto error;
    }

    if (db_info->thd != NULL)
    {
        ret = BK_FAIL;
        LOGE("%s, db_info->thd allready init, exit!\n", __func__);
        goto error;
    }

    ret = rtos_init_queue(&db_info->queue,
                          "db_info->queue",
                          sizeof(yangipc_msg_t),
                          10);

    if (ret != BK_OK)
    {
        LOGE("%s, ceate yangipc message queue failed\n");
        goto error;
    }

    ret = rtos_create_thread(&db_info->thd,
                             BEKEN_DEFAULT_WORKER_PRIORITY,
                             "db_info->thd",
                             (beken_thread_function_t)yangipc_message_handle,
                             1024 * 6,
                             NULL);

    if (ret != BK_OK)
    {
        LOGE("create media major thread fail\n");
        goto error;
    }

    yangipc_devices_init();
    yangipc_audio_device_init();

#if CONFIG_BLUETOOTH_HOST_ONLY
    yangipc_boarding_init();
#endif

    db_info->enabled = BK_TRUE;

    LOGD("%s success\n", __func__);

    return;

error:

    LOGE("%s fail\n", __func__);
}

