...
static void yangipc_wifi_event_cb(void *new_evt)
{
...
int yangipc_wifi_sta_connect(char *ssid, char *key)
{
    ...
    bk_wlan_status_register_cb(yangipc_wifi_event_cb);
...
int yangipc_socket_sendto(int *fd, const struct sockaddr *dst, uint8_t *data, uint32_t length, int offset)
{
    ...
    int max_retry = YANGIPC_SEND_MAX_RETRY;
    uint16_t index = 0;
    ...
    do
    {
        ...
        rtos_delay_milliseconds(YANGIPC_SEND_MAX_DELAY);
