...
#define YANGIPC_TCP_BUFFER (1460)
...
int yangipc_tcp_img_send_packet(uint8_t *data, uint32_t len)
{
    ...
    return yangipc_socket_sendto(&db_tcp_service->img_fd, (struct sockaddr *)&db_tcp_service->img_remote, data, len, -((int)sizeof(db_trans_head_t)));
}

int yangipc_tcp_img_send_prepare(uint8_t *data, uint32_t length)
{
    yangipc_transmission_pack(db_tcp_service->img_channel, data, length);

    return 0;
}

void *yangipc_tcp_img_get_tx_buf(void)
{
    if (db_tcp_service == NULL)
    {
        LOGE("yangipc_tcp_img_get_tx_buf, service null\n");
        return NULL;
    }

    if (db_tcp_service->img_channel == NULL)
    {
        LOGE("yangipc_tcp_img_get_tx_buf, img_channel null\n");
        return NULL;
    }

    LOGD("yangipc_tcp_img_get_tx_buf, tbuf %p\n", db_tcp_service->img_channel->tbuf);

    return db_tcp_service->img_channel->tbuf + 1;
}

int yangipc_tcp_img_get_tx_size(void)
{
    if (db_tcp_service == NULL)
    {
        LOGE("yangipc_tcp_img_get_tx_size, service null\n");
        return 0;
    }

    if (db_tcp_service->img_channel == NULL)
    {
        LOGE("yangipc_tcp_img_get_tx_size, img_channel null\n");
        return 0;
    }
...
int yangipc_tcp_aud_send_packet(uint8_t *data, uint32_t len)
{
    ...
    return yangipc_socket_sendto(&db_tcp_service->aud_fd, (struct sockaddr *)&db_tcp_service->aud_remote, data, len, -((int)sizeof(db_trans_head_t)));
}

int yangipc_tcp_aud_send_prepare(uint8_t *data, uint32_t length)
{
    yangipc_transmission_pack(db_tcp_service->aud_channel, data, length);

    return 0;
}

void *yangipc_tcp_aud_get_tx_buf(void)
{
    if (db_tcp_service == NULL)
    {
        LOGE("yangipc_tcp_aud_get_tx_buf, service null\n");
        return NULL;
    }

    if (db_tcp_service->aud_channel == NULL)
    {
        LOGE("yangipc_tcp_aud_get_tx_buf, aud_channel null\n");
        return NULL;
    }

    //LOGD("yangipc_tcp_aud_get_tx_buf, tbuf %p\n", db_tcp_service->aud_channel->tbuf);

    return db_tcp_service->aud_channel->tbuf + 1;
}

int yangipc_tcp_aud_get_tx_size(void)
{
    if (db_tcp_service == NULL)
    {
        LOGE("yangipc_tcp_aud_get_tx_size, service null\n");
        return 0;
    }

    if (db_tcp_service->aud_channel == NULL)
    {
        LOGE("yangipc_tcp_aud_get_tx_size, img_channel null\n");
        return 0;
    }
...
// V2版本：包装函数适配TCP图传接口
static frame_buffer_t *tcp_transmission_read_frame(image_format_t format, uint32_t timeout)
{
    // TCP传输是消费者，使用CONSUMER_TRANSMISSION ID
    // 标记为慢速消费者（受网络影响）
    return frame_queue_v2_get_frame(format, CONSUMER_TRANSMISSION, timeout);
}

static void tcp_transmission_free_frame(image_format_t format, frame_buffer_t *frame)
{
    // TCP传输释放帧
    frame_queue_v2_release_frame(format, CONSUMER_TRANSMISSION, frame);
}

static media_transfer_cb_t yangipc_tcp_img_channel =
{
    .send = yangipc_tcp_img_send_packet,
    .prepare = yangipc_tcp_img_send_prepare,
    .get_tx_buf = yangipc_tcp_img_get_tx_buf,
    .get_tx_size = yangipc_tcp_img_get_tx_size,
    .read = tcp_transmission_read_frame,
    .free = tcp_transmission_free_frame,
};

static const media_transfer_cb_t yangipc_tcp_aud_channel =
{
    .send = yangipc_tcp_aud_send_packet,
    .prepare = yangipc_tcp_aud_send_prepare,
    .get_tx_buf = yangipc_tcp_aud_get_tx_buf,
    .get_tx_size = yangipc_tcp_aud_get_tx_size,
};

static void yangipc_image_server_thread(beken_thread_arg_t data)
{
    int rcv_len = 0;
    ...
    rcv_buf = (u8 *) os_malloc((YANGIPC_TCP_BUFFER + 1) * sizeof(u8));
    ...
    db_tcp_service->img_socket.sin_port = htons(YANGIPC_TCP_IMG_PORT);
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (FD_ISSET(db_tcp_service->img_server_fd, &watchfd))
            {
                ...
                yangipc_socket_set_qos(db_tcp_service->img_fd, IP_QOS_PRIORITY_LOW);

                db_tcp_service->img_status = BK_TRUE;

                while (db_tcp_service->img_status == BK_TRUE)
                {
                    rcv_len = recv(db_tcp_service->img_fd, rcv_buf, YANGIPC_TCP_BUFFER, 0);
                    if (rcv_len > 0)
                    {
...
void yangipc_tcp_image_server_start(void)
{
    int ret;
    ...
    if (!db_tcp_service->img_thd)
    {
        ret = rtos_create_thread(&db_tcp_service->img_thd,
                                 4,
                                 "db_tcp_img_srv",
                                 (beken_thread_function_t)yangipc_image_server_thread,
                                 1024 * 3,
                                 (beken_thread_arg_t)NULL);
        if (ret != kNoErr)
        {
            LOGE("Error: failed to create yangipc tcp img server: %d\n", ret);
        }
    }
}

static inline void yangipc_tcp_voice_receiver(db_channel_t *channel, uint16_t sequence, uint16_t flags, uint32_t timestamp, uint8_t sequences, uint8_t *data, uint16_t length)
{
    LOGV("%s %d\n", __func__, length);
...
static void yangipc_audio_server_thread(beken_thread_arg_t data)
{
    int rcv_len = 0;
    //  struct sockaddr_in server;
    bk_err_t ret = BK_OK;
    ...
    db_tcp_service->aud_socket.sin_port = htons(YANGIPC_TCP_AUD_PORT);
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (FD_ISSET(db_tcp_service->aud_server_fd, &watchfd))
            {
                ...
                yangipc_socket_set_qos(db_tcp_service->aud_fd, IP_QOS_PRIORITY_HIGH);

                while (db_tcp_service->aud_status == BK_TRUE)
                {
                    rcv_len = recv(db_tcp_service->aud_fd, rcv_buf, YANGIPC_TCP_BUFFER, 0);
                    if (rcv_len > 0)
                    {
                        //bk_net_send_data(rcv_buf, rcv_len, TVIDEO_SND_TCP);
                        yangipc_transmission_unpack(db_tcp_service->aud_channel, rcv_buf, rcv_len, yangipc_tcp_voice_receiver);
                        //LOGD("got length: %d\n", rcv_len);
                    }
                    else
                    {
...
void yangipc_tcp_audio_server_start(void)
{
    int ret;
    ...
    if (!db_tcp_service->aud_thd)
    {
        ret = rtos_create_thread(&db_tcp_service->aud_thd,
                                 4,
                                 "db_tcp_aud_srv",
                                 (beken_thread_function_t)yangipc_audio_server_thread,
                                 1024 * 3,
                                 (beken_thread_arg_t)NULL);
        if (ret != kNoErr)
        {
            LOGE("Error: failed to create yangipc tcp aud server: %d\n", ret);
        }
    }
}


void yangipc_tcp_service_init(void)
{
    LOGD("%s, %d\n", __func__, __LINE__);
    ...
    db_tcp_service->img_channel = yangipc_transmission_malloc(0, YANGIPC_TCP_NETWORK_MAX_SIZE);
    db_tcp_service->aud_channel = yangipc_transmission_malloc(YANGIPC_TCP_NETWORK_MAX_SIZE, YANGIPC_TCP_NETWORK_MAX_SIZE);

    yangipc_devices_set_camera_transfer_callback(&yangipc_tcp_img_channel);

    yangipc_devices_set_audio_transfer_callback(&yangipc_tcp_aud_channel);

    yangipc_tcp_image_server_start();
    yangipc_tcp_audio_server_start();
