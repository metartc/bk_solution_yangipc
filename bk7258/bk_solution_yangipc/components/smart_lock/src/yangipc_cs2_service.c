...
static int32_t yangipc_cs2_p2p_write(int SessionID, uint8_t Channel, uint8_t *buff, uint32_t size);
...
yangipc_cs2_info_t *yangipc_cs2_info = NULL;
...
yangipc_cs2_h264_drop_info_t s_h264_drop_info = {0};
#endif
...
int yangipc_cs2_img_send_packet(uint8_t *data, uint32_t len)
{
    int send_byte = 0;
    uint32_t index = 0;
    ...
    if (!yangipc_cs2_info->img_status)
    {
    ...
    do
    {
        ...
        send_byte = yangipc_cs2_p2p_write(s_current_sessionid, IMG_P2P_CHANNEL, ptr + index, size - index);
...
void yangipc_cs2_img_timer_deinit(void)
{
    if (yangipc_cs2_info)
    {
        rtos_lock_mutex(&yangipc_cs2_info->mutex);
        ...
        rtos_unlock_mutex(&yangipc_cs2_info->mutex);
    }
}

int yangipc_cs2_img_send_prepare(uint8_t *data, uint32_t length)
{
    yangipc_transmission_pack(yangipc_cs2_info->img_channel, data, length);

    return 0;
}

#if CONFIG_MEDIA_DROP_STRATEGY_ENABLE
void yangipc_cs2_img_h264_record_map(bool is_I_frame)
{
...
void yangipc_cs2_h264_down_drop_level(uint32_t cur_drop_level, bool recovery, bool is_I_frame)
{
...
bool yangipc_cs2_img_h264_drop_level_check(uint32_t pre_size, UINT32 WriteSize, bool is_I_frame)
{
    ...
    if (need_drop)
    {
        ...
        {
            yangipc_cs2_h264_down_drop_level(cur_drop_level, false, is_I_frame);
        }
        else
        {
    ...
    {
        yangipc_cs2_h264_down_drop_level(s_h264_drop_info.drop_level, true, is_I_frame);
    }
...
bool yangipc_cs2_img_h264_drop_check(frame_buffer_t *frame, uint32_t count, uint16_t ext_size)
{
    int32_t Check_ret = 0;
    UINT32 WriteSize = 0;
    ...
    yangipc_cs2_img_h264_record_map(is_NAL_I_frame);
    ...
    drop_other_gop_frame = yangipc_cs2_img_h264_drop_level_check(pre_size, WriteSize, is_NAL_I_frame);

    return drop_other_gop_frame;

}
bool yangipc_cs2_img_drop_check(frame_buffer_t *frame, uint32_t count, uint16_t ext_size)
{
    bool ret = false;
    ...
    {
        case PIXEL_FMT_H264:
            ret = yangipc_cs2_img_h264_drop_check(frame, count, ext_size);
            break;
        default:
            break;
    }
...
void *yangipc_cs2_img_get_tx_buf(void)
{
    if (yangipc_cs2_info == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_buf, service null\n");
        return NULL;
    }

    if (yangipc_cs2_info->img_channel == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_buf, img_channel null\n");
        return NULL;
    }

    LOGD("yangipc_udp_img_get_tx_buf, tbuf %p\n", yangipc_cs2_info->img_channel->tbuf);

    return yangipc_cs2_info->img_channel->tbuf + 1;
}

int yangipc_cs2_img_get_tx_size(void)
{
    if (yangipc_cs2_info == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_size, service null\n");
        return 0;
    }

    if (yangipc_cs2_info->img_channel == NULL)
    {
        LOGE("yangipc_udp_img_get_tx_size, img_channel null\n");
        return 0;
    }

    return yangipc_cs2_info->img_channel->tsize - sizeof(db_trans_head_t);
}

int yangipc_cs2_aud_send_packet(uint8_t *data, uint32_t len)
{
    int send_byte = 0;
    uint32_t index = 0;
    uint8_t *ptr = data - sizeof(db_trans_head_t);
    ...
    if (!yangipc_cs2_info->aud_status)
    {
    ...
    do
    {
        send_byte = yangipc_cs2_p2p_write(s_current_sessionid, AUD_P2P_CHANNEL, ptr + index, size - index);
...
int yangipc_cs2_aud_send_prepare(uint8_t *data, uint32_t length)
{
    yangipc_transmission_pack(yangipc_cs2_info->aud_channel, data, length);
    return 0;
}


void *yangipc_cs2_aud_get_tx_buf(void)
{
    if (yangipc_cs2_info == NULL)
    {
        LOGE("yangipc_cs2_aud_get_tx_buf, service null\n");
        return NULL;
    }

    if (yangipc_cs2_info->aud_channel == NULL)
    {
        LOGE("yangipc_cs2_aud_get_tx_buf, aud_channel null\n");
        return NULL;
    }

    //LOGD("yangipc_cs2_aud_get_tx_buf, tbuf %p\n", yangipc_cs2_info->aud_channel->tbuf);

    return yangipc_cs2_info->aud_channel->tbuf + 1;
}


int yangipc_cs2_aud_get_tx_size(void)
{
    if (yangipc_cs2_info == NULL)
    {
        LOGE("yangipc_cs2_aud_get_tx_size, service null\n");
        return 0;
    }

    if (yangipc_cs2_info->aud_channel == NULL)
    {
        LOGE("yangipc_cs2_aud_get_tx_size, aud_channel null\n");
        return 0;
    }

    return yangipc_cs2_info->aud_channel->tsize - sizeof(db_trans_head_t);
}
...
static media_transfer_cb_t yangipc_cs2_img_channel =
{
    .send = yangipc_cs2_img_send_packet,
    .prepare = yangipc_cs2_img_send_prepare,
    .get_tx_buf = yangipc_cs2_img_get_tx_buf,
    .get_tx_size = yangipc_cs2_img_get_tx_size,
    .read = cs2_transmission_read_frame,
    .free = cs2_transmission_free_frame,
#if CONFIG_MEDIA_DROP_STRATEGY_ENABLE
    .drop_check = yangipc_cs2_img_drop_check,
#else
    .drop_check = NULL,
#endif
};
...
static const media_transfer_cb_t yangipc_cs2_aud_channel =
{
    .send = yangipc_cs2_aud_send_packet,
    .prepare = yangipc_cs2_aud_send_prepare,
    .get_tx_buf = yangipc_cs2_aud_get_tx_buf,
    .get_tx_size = yangipc_cs2_aud_get_tx_size,
};

static void yangipc_cs2_get_time(time_info_t *pt)
{
...
static int32_t yangipc_cs2_p2p_cmd_handle(uint8_t *data, uint32_t length)
{
    LOGD("%s, length: %d\n", __func__, length);
    yangipc_transmission_unpack(yangipc_cs2_info->db_channel, data, length, yangipc_transmission_cmd_recive_callback);
    return 0;
}

static int yangipc_cs2_p2p_event_send(uint8_t *data, uint16_t length)
{
    int send_byte = 0;
    uint32_t index = 0;
    ...
    do
    {
        send_byte = yangipc_cs2_p2p_write(s_current_sessionid, CMD_P2P_CHANNEL, data + index, length - index);
...
static inline void yangipc_cs2_voice_receiver(db_channel_t *channel, uint16_t sequence, uint16_t flags, uint32_t timestamp, uint8_t sequences, uint8_t *data, uint16_t length)
{
    yangipc_audio_data_callback(data, length);
}


static void yangipc_cs2_session_close(void)
{
    time_info_t t1, t2;
...
out:
    ...
    if (yangipc_cs2_info->device_connected == BK_TRUE)
    {
        yangipc_msg_t msg;
        ...
        yangipc_cs2_info->device_connected = BK_FALSE;
    }
...
static int yangipc_cs2_p2p_audio_receiver(beken_thread_arg_t arg)
{
    int32_t ret = 0;
    ...
    rtos_set_semaphore(&yangipc_cs2_info->aud_sem);

    while (yangipc_cs2_info->aud_running == BK_TRUE)//Repeat < Total_Times)
    {
        ...
        if (0 <= s_current_sessionid)
        {
            ...
            rtos_lock_mutex(&yangipc_cs2_info->mutex);
            yangipc_cs2_info->aud_status = BK_TRUE;
            rtos_unlock_mutex(&yangipc_cs2_info->mutex);
            ...
            do
            {
                ...
                if (ReadSize)
                {
                    ...
                    yangipc_transmission_unpack(yangipc_cs2_info->aud_channel, tmp_read_buf, ReadSize, yangipc_cs2_voice_receiver);
                ...
                if (ret < 0 && ERROR_PPCS_TIME_OUT != ret)
                {
                    ...
                    yangipc_cs2_session_close();
                    break;
                }
            }
            while (yangipc_cs2_info->aud_running == BK_TRUE);

            yangipc_cs2_info->aud_status = BK_FALSE;
            yangipc_cs2_info->aud_running = false;
...
static int yangipc_cs2_p2p_interface_core(p2p_cs2_key_t *key)
{
    int Repeat = 0;
    int32_t ret = 0;
    ...
    while (yangipc_cs2_info->is_running)//Repeat < Total_Times)
    {
        Repeat++;
        ...
        s_current_sessionid = session = cs2_p2p_listen(key->did, key->apilicense, Repeat, &yangipc_cs2_info->is_running);

        if (0 <= session)
        {
            BK_LOGD("p2p", "%s listen Sid %d\n", __func__, s_current_sessionid);

            yangipc_cs2_info->device_connected = BK_TRUE;

            yangipc_cs2_info->img_status = BK_TRUE;
            yangipc_cs2_info->aud_status = BK_TRUE;
            ...
            do
            {
                ...
                if (ReadSize)
                {
                    ...
                    yangipc_cs2_p2p_cmd_handle(tmp_read_buf, ReadSize);
                    continue;
                }
            ...
            while (yangipc_cs2_info->is_running);
READ_ERR:
        ...
        yangipc_cs2_info->img_status = BK_FALSE;
        yangipc_cs2_info->aud_status = BK_FALSE;
...
static int yangipc_cs2_p2p_interface_init(p2p_cs2_key_t *key)
{
    int ret = 0;
...
static void yangipc_cs2_p2p_interface_deinit(void)
{
...
static void yangipc_cs2_service_main(beken_thread_arg_t arg)
{
    yangipc_cs2_p2p_interface_init(yangipc_cs2_info->cs2_key);

    yangipc_cs2_info->is_running = 1;

    yangipc_cs2_p2p_interface_core(yangipc_cs2_info->cs2_key);

    yangipc_cs2_p2p_interface_deinit();
}


static const db_channel_cb_t db_channel_callback =
{
    .tsend = yangipc_cs2_p2p_event_send,
};


int yangipc_cs2_service_init(void *param)
{
    int ret;
    ...
    if (yangipc_cs2_info == NULL)
    {
        yangipc_cs2_info = os_malloc(sizeof(yangipc_cs2_info_t));

        if (yangipc_cs2_info == NULL)
        {
            LOGE("%s yangipc_cs2_info malloc failed\n", __func__);
            return BK_FAIL;
        }

    }

    os_memset(yangipc_cs2_info, 0, sizeof(yangipc_cs2_info_t));

    yangipc_cs2_info->cs2_key = p2p_cs2_key;

    rtos_init_mutex(&yangipc_cs2_info->tx_lock);

    rtos_init_mutex(&yangipc_cs2_info->mutex);

    yangipc_cs2_info->db_channel = yangipc_transmission_malloc(YANGIPC_NETWORK_MAX_SIZE, YANGIPC_NETWORK_MAX_SIZE);
    yangipc_cs2_info->db_channel->cb = &db_channel_callback;


    yangipc_cs2_info->img_channel = yangipc_transmission_malloc(0, YANGIPC_TCP_NETWORK_MAX_SIZE);
    ...
    yangipc_cs2_info->aud_channel = yangipc_transmission_malloc(1600, YANGIPC_TCP_NETWORK_MAX_SIZE);
    ...
    ret = rtos_init_semaphore_ex(&yangipc_cs2_info->aud_sem, 1, 0);

    yangipc_devices_set_camera_transfer_callback(&yangipc_cs2_img_channel);
    yangipc_devices_set_audio_transfer_callback(&yangipc_cs2_aud_channel);

    ret = rtos_create_thread(&yangipc_cs2_info->thd,
                             4,
                             "yangipc",
                             (beken_thread_function_t)yangipc_cs2_service_main,
                             1024 * 8,
                             (beken_thread_arg_t)NULL);
    if (ret != BK_OK)
    {
        LOGE("Error: Failed to create yangipc udp service: %d\n", ret);
        return BK_FAIL;
    }
...
void demo_yangipc_cs2_p2p_server_deinit(void)
{
    LOGD("%s\n", __func__);
}


static int yangipc_cs2_camera_state_changed(yangipc_state_t state)
{

    return BK_OK;
}

static int yangipc_cs2_audio_state_changed(yangipc_state_t state)
{
    int ret;
    ...
    if (state == DB_TURN_ON)
    {
        if (yangipc_cs2_info->aud_running == BK_TRUE)
        {
            LOGW("%s already open\n", __func__);
            return BK_OK;
        }
        ...
        yangipc_cs2_info->aud_running = BK_TRUE;

        ret = rtos_create_thread(&yangipc_cs2_info->thd,
                                 4,
                                 "audo",
                                 (beken_thread_function_t)yangipc_cs2_p2p_audio_receiver,
                                 1024 * 4,
                                 (beken_thread_arg_t)NULL);
        ...
        ret = rtos_get_semaphore(&yangipc_cs2_info->aud_sem, BEKEN_WAIT_FOREVER);
    ...
    {
#if 0
        if (yangipc_cs2_info->aud_running == BK_FALSE)
        {
            LOGW("%s already close\n", __func__);
            return BK_OK;
        }
        ...
        yangipc_cs2_info->aud_running = BK_FALSE;
...
const yangipc_service_interface_t yangipc_cs2_service =
{
    .init = yangipc_cs2_service_init,
    .deinit = demo_yangipc_cs2_p2p_server_deinit,
    .camera_state_changed = yangipc_cs2_camera_state_changed,
    .audio_state_changed = yangipc_cs2_audio_state_changed,
    .camera_transfer_cb = &yangipc_cs2_img_channel,
    .audio_transfer_cb = &yangipc_cs2_aud_channel,
};

const yangipc_service_interface_t *get_yangipc_cs2_service_interface(void)
{
    return &yangipc_cs2_service;
}
