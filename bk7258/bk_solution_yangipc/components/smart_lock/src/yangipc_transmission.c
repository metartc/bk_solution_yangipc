...
{
    struct timeval tv;
} yangipc_trans_info_t;

yangipc_trans_info_t g_yangipc_trans_info = {0};
/*******************************************************************************
* crc8
*
...
void yangipc_hex_dump(uint8_t *data, uint32_t length)
{
#ifdef DUMP_DEBUG
...
db_channel_t *yangipc_transmission_malloc(uint16_t max_rx_size, uint16_t max_tx_size)
{
...
void yangipc_transmission_unpack(db_channel_t *channel, uint8_t *data, uint32_t length, yangipc_transmission_recive_cb_t cb)
{
    ...
    while (left != 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                yangipc_hex_dump(channel->cbuf, channel->ccount);
                //TODO FIXME
                break;
            }
...
uint32_t yangipc_transmission_get_milliseconds(void)
{
    uint32_t time = 0;
    ...
    bk_rtc_gettimeofday(&(g_yangipc_trans_info.tv), 0);

    long time_offset = 8 * 3600; //8 hour offset
    long ms_time = (g_yangipc_trans_info.tv.tv_usec / 1000);
    long time_ms = 0;

    if (g_yangipc_trans_info.tv.tv_sec < time_offset)
    {
        //LOGD(" local time sec: %ld \r\n",g_yangipc_trans_info.tv.tv_sec);
        time_ms = (g_yangipc_trans_info.tv.tv_sec * 1000) + ms_time;
    }
    else
    {
        time_ms = ((g_yangipc_trans_info.tv.tv_sec - time_offset) * 1000) + ms_time;
    }

    time = (time_ms & 0xFFFFFFFF);

    //LOGD("sec: %ld time %d\r\n", g_yangipc_trans_info.tv.tv_sec ,time);
#endif

    return time;
}

void yangipc_transmission_pack(db_channel_t *channel, uint8_t *data, uint32_t length)
{
    ...
    */
    ...
    head->timestamp = CHECK_ENDIAN_UINT32(yangipc_transmission_get_milliseconds());
...
int yangipc_transmission_pack_send(db_channel_t *channel, uint8_t *data, uint32_t length, yangipc_transmission_send_t cb)
{
    bk_err_t ret = BK_FAIL;

    yangipc_transmission_pack(channel, data, length);
