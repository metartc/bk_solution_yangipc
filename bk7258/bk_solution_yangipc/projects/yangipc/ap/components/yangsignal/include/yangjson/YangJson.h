...
typedef struct{
	YangJson* session;
	int32_t (*getArraySize)(const YangJson *array);
	YangJson * (*getArrayItem)(const YangJson *array, int32_t index);
...
typedef struct{
	...
	char * (*printBuffered)(YangJson *item, int32_t prebuffer, yangbool fmt);
