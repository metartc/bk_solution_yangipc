...
typedef struct{
	void* session;
	void (*mqtt_receive)(void* session,char* topic,char* msg,int32_t msgLen);
	void (*mqtt_connect_error)(void* session,int32_t  errCode);
}YangMqttCallback;

typedef struct{
	void* session;
	int32_t  (*connect)(void* session,yangbool isSyn,int32_t  sendBufferSize,int32_t  recvBufferSize,char *remoteIp,int32_t  remotePort,char* userName,char* password);
	int32_t  (*ping)(void* psession);
	int32_t  (*reconnect)(void* psession);
	int32_t  (*reconnect2)(void* psession);
	int32_t  (*synMessage)(void* psession);
...
extern "C"{
#endif
	int32_t  yang_create_mqtt(YangMqtt* mqtt,YangMqttCallback* mqttcb,yangbool enableTls);
