...
int32_t yang_ipcmsg_parse(char* msg,YangIpcRequest* request){
	...
	reader=(YangJsonReader*)yang_calloc(sizeof(YangJsonReader),1);
	...
	{
		request->connectType=connectType->valueint;
	}

	if (reader->isNumber(requestType))
	{
		request->requestType=requestType->valueint;
	}
...
int32_t yang_ipcmsg_send(YangMqtt* mqtt,char* topic,YangIpcRequest* request){
	YangJsonWriter* writer=(YangJsonWriter*)yang_calloc(sizeof(YangJsonWriter),1);
